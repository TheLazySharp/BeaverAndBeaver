class_name  StoneAmmo
extends Area2D

@export var stone_data: WeaponData
...
@onready var slingshot: Node2D = $/root/World/BeaverSr/Weapons/Slingshot
...
func _ready() -> void:
	...
	speed = stone_data.speed
	max_range = stone_data.atk_range
	max_lvl = stone_data.max_level

func _process(_delta: float) -> void:
	current_lvl = clampi(stone_data.current_level,0,max_lvl)
	damages = stone_data.dmg + (current_lvl * .1 * 28) #améliorer la formule d'augmentation des dégats
...
func desactivate()-> void:
	...
	if slingshot: slingshot.add_bullet_to_pool(self)
